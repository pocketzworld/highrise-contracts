...
def generate_fund_request(
    token_ids: list[int], expiry: int, cost: int, key: str
) -> tuple[bytes, bytes]:
    payload = encode_abi(["uint256[]", "uint256", "uint256"], [token_ids, expiry, cost])
    hash = keccak(payload)
...
def test_can_fund(
    ...
    payload, sig = generate_fund_request(
        [token_id, 20], expiry, price, admin.private_key
    )
    ...
    assert tx.events[0]["tokenId"] == 15
    assert tx.events[1]["to"] == alice
    assert tx.events[1]["tokenId"] == 20
...
def test_modifier_enabled(
    ...
    price = get_wei_land_price()
    payload, sig = generate_fund_request(
        [1], int(time() + 10), price, admin.private_key
    )
...
def test_withdraw(
    ...
    price = get_wei_land_price()
    payload, sig = generate_fund_request(
        [10], int(time() + 10), price, admin.private_key
    )
    # Fund the contract
    enabled_land_funding_contract.fund(
        payload,
