...
from brownie import config, network, project
...
def deploy():
    ...
    proxy_admin = deploy_proxy_admin(oz, account)
    opensea_address = opensea_proxy_registry_address(account)
    land_proxy, land = deploy_land(
        oz, proxy_admin, opensea_address, environment, account
    )
    estate_proxy, estate = deploy_estate(
        oz, proxy_admin, land_proxy.address, opensea_address, environment, account
    )

    if config["networks"][network.show_active()].get("verify"):
        verify_land(land.address)
        verify_proxy(oz, land_proxy.address)
        verify_estate(estate.address)
        verify_estate_proxy(oz, estate_proxy.address)
