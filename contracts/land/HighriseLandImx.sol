...
import "../../interfaces/imx/IMintable.sol";
import "../opensea/Utils.sol";
import "../imx/Minting.sol";

contract HighriseLandImx is
    Initializable,
    ERC721Upgradeable,
    ...
    IHighriseLand
{
    ...
    // ------------------------------ INIT IMX -----------------------------------------------------------------------------
    function initIMX(address imxAddress) public  {
        _imx = imxAddress;
    }
    ...
    modifier onlyIMX() {
        require(msg.sender == _imx, "Function can only be called by imx");
        _;
    }


    function mintFor(address user, uint256 quantity, bytes calldata mintingBlob) external onlyIMX {
    ...
    function _mintFor(address to, uint256 id, bytes memory) internal {
        _safeMint(to, id);
    }
    // ---------------------------------------------------------------------------------
}
